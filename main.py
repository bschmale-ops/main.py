...
def create_match_alert(match, time_until):
    ...
    embed = discord.Embed(
        ...
        description=f"# {team1_display}\n# <:VS:1428145739443208305>\n# {team2_display}\n",  # ⬅️ Absatz nach Vitality
        color=0x00ff00 if time_until > 15 else 0xff9900,
        timestamp=datetime.datetime.now()
    )
    
    # Time rechtsbündig mit Spacer
    time_with_spacer = f"{'\u2800' * 15}{match['time_string']}"
    
    embed.add_field(name="🏆 Tournament", value=match['event'], inline=True)
    embed.add_field(name="🕐 Time", value=time_with_spacer, inline=True)  # ⬅️ Rechtsbündig
    embed.add_field(name="", value="", inline=False)  # ⬅️ Absatz nach Tournament/Time
    embed.add_field(name="⏰ Starts in", value=f"**{int(time_until)} minutes**", inline=False)
    embed.add_field(name="", value="", inline=False)  # ⬅️ Absatz nach Starts in
